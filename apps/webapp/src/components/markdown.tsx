...
export const Markdown = ({ markdown }: { markdown: string }) => {
  return (
    <ReactMarkdown
      ...
      components={{
        p: ({ children }) => <p className='text-lg font-medium leading-[26px]'>{children}</p>,
        h1: ({ children }) => (
        ...
        h3: ({ children }) => <h3 className='text-[1.25em] leading-tight'>{children}</h3>,
        h4: ({ children }) => <h4 className='text-[1em] leading-tight'>{children}</h4>,
        h5: ({ children }) => <h5 className='text-[.875em] leading-tight'>{children}</h5>,
        h6: ({ children }) => <h6 className='text-[.85em] leading-tight'>{children}</h6>,
        ol: ({ children }) => {
          return (
        ...
        ul: ({ children }) => <ul className='mt-3 list-inside list-none space-y-3'>{children}</ul>,
        li: ({ children }) => (
          <li className='flex [&>ul>li]:text-base [&>ul>li]:font-medium [&>ul>li]:text-muted-foreground [&>ul]:w-full'>
            <div className='txt-inherit flex flex-col justify-center text-inherit'>{children}</div>
          </li>
        ),
        a: ({ children, href }) => (
          <a target='_blank' rel='noopener noreferrer' href={href} className='break-all underline'>
            {children}
          </a>
        ),
        code: ({ children }) => (
          <code className='rounded-[6px] bg-[#6e768166] p-1 text-[85%]'>{children}</code>
        ),
        pre: ({ children }) => {
          const content = (
            children as {
